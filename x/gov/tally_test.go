...
func TestTallyDelgatorMultipleInherit(t *testing.T) {
	...
	val1CreateMsg := staking.NewMsgCreateValidator(
		sdk.ValAddress(input.addrs[0]), ed25519.GenPrivKey().PubKey(), sdk.NewCoin(sdk.DefaultBondDenom, valTokens1), testDescription, testCommissionRates, sdk.OneInt(), "test1",
	)
	stakingHandler(ctx, val1CreateMsg)

	valTokens2 := sdk.TokensFromConsensusPower(6)
	val2CreateMsg := staking.NewMsgCreateValidator(
		sdk.ValAddress(input.addrs[1]), ed25519.GenPrivKey().PubKey(), sdk.NewCoin(sdk.DefaultBondDenom, valTokens2), testDescription, testCommissionRates, sdk.OneInt(), "test2",
	)
	stakingHandler(ctx, val2CreateMsg)

	valTokens3 := sdk.TokensFromConsensusPower(7)
	val3CreateMsg := staking.NewMsgCreateValidator(
		sdk.ValAddress(input.addrs[2]), ed25519.GenPrivKey().PubKey(), sdk.NewCoin(sdk.DefaultBondDenom, valTokens3), testDescription, testCommissionRates, sdk.OneInt(), "test3",
	)
