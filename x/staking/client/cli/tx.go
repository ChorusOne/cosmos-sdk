...
func PrepareFlagsForTxCreateValidator(
	...
	if viper.GetString(FlagSharesDenomPrefix) == "" {
		viper.Set(FlagSharesDenomPrefix, strings.ToLower(config.Moniker[0:int(math.Min(float64(len(config.Moniker)), 6.00))]))
	}
}
...
func BuildCreateValidatorMsg(cliCtx context.CLIContext, txBldr auth.TxBuilder) (auth.TxBuilder, sdk.Msg, error) {
	...
	msg := types.NewMsgCreateValidator(
		sdk.ValAddress(valAddr), pk, amount, description, commissionRates, minSelfDelegation, sharesDenomPrefix,
	)
